...
for (const relativeFile of operationalFiles) {
  ...
  if (forbiddenWrapperExtensions.has(extension)) {
    violations.push(`${relativeFile}: envoltorio especifico de Windows`);
    continue;
  }

  const contents = await readFile(path.join(projectRoot, relativeFile), "utf8");
  if (platformExecutablePattern.test(contents)) {
    violations.push(`${relativeFile}: ejecutable con extension de plataforma`);
  }
  if (windowsAbsolutePathPattern.test(contents)) {
    violations.push(`${relativeFile}: ruta absoluta especifica de Windows`);
  }
}
...
if (violations.length > 0) {
  console.error("El control de portabilidad encontro dependencias operativas:");
